...
final class UsageServiceTests: XCTestCase {
    ...
    func testBackoffIntervalWithNilRetryAfterDoublesInterval() {
        XCTAssertEqual(
            UsageService.backoffInterval(retryAfter: nil, currentInterval: 5 * 60),
            10 * 60
        )
    }

    func testBackoffIntervalRespectsRetryAfterWhenLargerThanDoubled() {
        XCTAssertEqual(
            UsageService.backoffInterval(retryAfter: 25 * 60, currentInterval: 5 * 60),
            25 * 60
        )
    }
    ...
    func testSignOutClearsStateAndCredentials() async throws {
        let store = try makeStore()
        try store.save(
            StoredCredentials(
                accessToken: "access",
    ...
    func testFetchProfileUsesLocalLoaderWhenAvailable() async throws {
        let store = try makeStore()
        try store.save(
            StoredCredentials(
                accessToken: "access",
        ...
        MockURLProtocol.handler = { _ in
            XCTFail("Should not make network request when local loader provides email")
        ...
        let service = UsageService(
            ...
            tokenEndpoint: URL(string: "https://example.com/v1/oauth/token")!,
            credentialsStore: store,
            localProfileLoader: { "local@example.com" }
        )

        await service.fetchProfile()
        XCTAssertEqual(service.accountEmail, "local@example.com")
    }

    func testBase64URLEncodingReplacesSpecialCharacters() {
        // Data that produces +, /, and = in standard base64
        let data = Data([251, 255, 190]) // base64 = "u/++", base64url = "u_--"
    ...
    func testFetchUsageWithNilCredentialsDoesNotCrash() async throws {
        let store = try makeStore()
        // No credentials saved

        MockURLProtocol.handler = { _ in
            XCTFail("Should not make network request without credentials")
            throw URLError(.badServerResponse)
        }
