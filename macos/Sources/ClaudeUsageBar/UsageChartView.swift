...
struct UsageChartView: View {
    ...
    private func chartView(points: [UsageDataPoint]) -> some View {
        ...
        Chart {
            ...
            if hasSonnetData(points) {
                ForEach(points) { point in
                    if let sonnet = point.pctSonnet7d {
                        LineMark(
                            x: .value("Time", point.timestamp),
                            y: .value("Usage", sonnet * 100)
                        )
                        .foregroundStyle(by: .value("Window", "Sonnet 7d"))
                        .interpolationMethod(.catmullRom)
                    }
                }
            }
    ...
    @ViewBuilder
    private func tooltipView(date: Date, pct5h: Double, pct7d: Double, pctSonnet7d: Double? = nil) -> some View {
        VStack(spacing: 2) {
            ...
            HStack(spacing: 6) {
                ...
                Label("\(Int(round(pct7d * 100)))%", systemImage: "circle.fill")
                    .font(.system(size: 9, weight: .medium))
                    .foregroundStyle(.orange)
                if let sonnet = pctSonnet7d {
                    Label("\(Int(round(sonnet * 100)))%", systemImage: "circle.fill")
                        .font(.system(size: 9, weight: .medium))
                        .foregroundStyle(.green)
                }
            }
    ...
    private func hasSonnetData(_ points: [UsageDataPoint]) -> Bool {
        points.contains { $0.pctSonnet7d != nil }
    }
...
enum UsageChartInterpolation {
    ...
    static func interpolateValues(at date: Date, in points: [UsageDataPoint]) -> UsageChartInterpolatedValues? {
        ...
        if date < sorted.first!.timestamp || date > sorted.last!.timestamp {
            return UsageChartInterpolatedValues(date: date, pct5h: 0, pct7d: 0, pctSonnet7d: nil)
        }
        ...
        for i in 0..<(sorted.count - 1) {
            if date >= sorted[i].timestamp && date <= sorted[i + 1].timestamp {
                ...
                let pctSonnet7d: Double?
                if let s0 = sorted[i0].pctSonnet7d,
                   let s1 = sorted[i].pctSonnet7d,
                   let s2 = sorted[i + 1].pctSonnet7d,
                   let s3 = sorted[i3].pctSonnet7d {
                    pctSonnet7d = clampToUnitInterval(catmullRom(s0, s1, s2, s3, t: t))
                } else {
                    pctSonnet7d = nil
                }
