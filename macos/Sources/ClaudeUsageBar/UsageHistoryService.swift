...
class UsageHistoryService: ObservableObject {
    ...
    func recordDataPoint(pct5h: Double, pct7d: Double, pctSonnet7d: Double? = nil) {
        let point = UsageDataPoint(pct5h: pct5h, pct7d: pct7d, pctSonnet7d: pctSonnet7d)
    ...
    func downsampledPoints(for range: TimeRange) -> [UsageDataPoint] {
        ...
        return buckets.compactMap { bucket -> UsageDataPoint? in
            ...
            let sonnetValues = bucket.compactMap(\.pctSonnet7d)
            let avgSonnet7d: Double? = sonnetValues.isEmpty ? nil : sonnetValues.reduce(0, +) / Double(sonnetValues.count)
